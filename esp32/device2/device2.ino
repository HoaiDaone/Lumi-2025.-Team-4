...
String my_device_id = "02";
String name_device = "esp32_02";
    ...
    if (!sendWasDown) {
      ...
      if (state == STATE_MAIL_WRITE && write_len > 0) {
        ...
        if (checkDeviceIdExists(id)) {
          ...
          drawMailList();
          beep(100);
          // Tạo hội thoại rỗng ở cả hai thiết bị
          createEmptyConversation(my_device_id, id);
        } else {
...
// Hàm tạo hội thoại rỗng ở cả hai thiết bị
void createEmptyConversation(const String &fromId, const String &toId) {
  // Gửi tin nhắn rỗng sang thiết bị kia để tạo hội thoại ở cả hai bên
  sendMessageToServer(fromId, toId, "");
}
...
void setup() {
  ...
  // Tạo sẵn một cuộc hội thoại với thiết bị còn lại (ví dụ: "01") nếu chưa có
  String other_id = "01";
  bool found = false;
  for (int i = 0; i < mail_count; i++) {
    if (mail_ids[i] == other_id) {
      found = true;
      break;
    }
  }
  if (!found && my_device_id != other_id && checkDeviceIdExists(other_id)) {
    if (mail_count < MAX_MAILS) {
      mail_ids[mail_count] = other_id;
      convs[mail_count].size = 0;
      mail_count++;
      mail_index = mail_count - 1;
      createEmptyConversation(my_device_id, other_id);
    }
  }
}
